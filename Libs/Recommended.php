...
if ( ! class_exists( 'Recommended' ) ) {
	...
	class Recommended {
		...
		public function __construct( $menu_order = 99 ) {
			...
			add_action( 'wp_ajax_jltwporgst_recommended_upgrade_plugin', array( $this, 'jltwporgst_recommended_upgrade_plugin' ) );
			add_action( 'wp_ajax_jltwporgst_recommended_activate_plugin', array( $this, 'jltwporgst_recommended_activate_plugin' ) );
		}
		...
		public function render_recommended_plugins() { ?>
			<div class='jltwporgst-recommended-wrapper'>
				<?php $this->header(); ?>
				<?php $this->body(); ?>
			</div>
			<?php
		...
		public function header() {
			?>
			<div class='jltwporgst-recommended-header'>
				<div class='jltwporgst-recommended-title'>
					<h2>
						<?php echo esc_html__( 'Recommended Plugins', 'wp-org-plugin-stats' ); ?>
					</h2>
				</div>
				<div class='jltwporgst-recommended-menu'>
					<div class="wp-filter">
						...
						<form class="search-form jltwporgst-search-plugins mr-0" method="get">
							<input type="hidden" name="tab" value="search">
							<label class="screen-reader-text" for="search-plugins">
								<?php echo esc_html__( 'Search Plugins', 'wp-org-plugin-stats' ); ?>
							</label>
							<input type="search" name="s" id="search-plugins" value="" class="wp-filter-search" placeholder="<?php echo esc_html__( 'Search plugins...', 'wp-org-plugin-stats' ); ?>">
							<input type="submit" id="search-submit" class="button hide-if-js" value="<?php echo esc_html__( 'Search Plugins', 'wp-org-plugin-stats' ); ?>">
						</form>
					</div>
				</div>
			</div>
					...
					<div class="plugin-card-bottom">
						<div class="column-downloaded">
							<span class="plugin-status">
								<?php
								echo esc_html__( 'Status:', 'wp-org-plugin-stats' );

								if ( 'install' === $install_status['status'] ) {
									?>
									<span class="plugin-status-not-install" data-plugin-url="<?php echo esc_attr( $plugin['download_link'] ); ?>"><?php echo esc_html__( 'No Installed', 'wp-org-plugin-stats' ); ?></span>
									<?php
								} elseif ( 'update_available' === $install_status['status'] ) {
									if ( is_plugin_active( $install_status['file'] ) ) {
										?>
										<span class="plugin-status-active">
											<?php echo esc_html__( 'Active', 'wp-org-plugin-stats' ); ?>
										</span>
										<?php
									} else {
										?>
										<span class="plugin-status-inactive" data-plugin-file="<?php echo esc_attr( esc_attr( $install_status['file'] ) ); ?>">
											<?php echo esc_html__( 'Inactive', 'wp-org-plugin-stats' ); ?>
										</span>
										<?php
									}
								...
								if ( 'install' === $install_status['status'] ) {
									...
									<li class="mr-0">
										<button class="install-now button button-primary" data-install-url="<?php echo esc_attr( $plugin['download_link'] ); ?>">
											<?php echo esc_html__( 'Install Now', 'wp-org-plugin-stats' ); ?>
										</button>
									</li>
									<?php
								} elseif ( 'update_available' === $install_status['status'] ) {
									...
									<li class="mr-0">
										<button class="update-now button" data-plugin="<?php echo esc_attr( $install_status['file'] ); ?>" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-update-url="<?php echo esc_attr( $install_status['url'] ); ?>">
											<?php echo esc_html__( 'Update Now', 'wp-org-plugin-stats' ); ?>
										</button>
									</li>
									<?php
								} elseif ( ( 'latest_installed' === $install_status['status'] ) || ( 'newer_installed' === $install_status['status'] ) ) {
									if ( is_plugin_active( $install_status['file'] ) ) {
										...
										<li class="mr-0">
											<button type="button" class="button button-disabled" disabled="disabled">
												<?php echo esc_html__( 'Activated', 'wp-org-plugin-stats' ); ?>
											</button>
										</li>
										<?php
									} elseif ( current_user_can( 'activate_plugin', $install_status['file'] ) ) {
										...
										<button class="button activate-now" data-plugin-file="<?php echo esc_attr( $install_status['file'] ); ?>">
											<?php echo esc_html__( 'Activate', 'wp-org-plugin-stats' ); ?>
										</button>
										<?php
									} else {
										?>
										<li class="mr-0">
											<button type="button" class="button button-disabled" disabled="disabled">
												<?php echo esc_html__( 'Installed', 'wp-org-plugin-stats' ); ?>
											</button>
										</li>
										<?php
									}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltwporgst_recommended_activate_plugin() {
			try {
				if ( isset( $_POST['file'] ) ) {
					$nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';

					if ( ! wp_verify_nonce( $nonce, 'jltwporgst_recommended_nonce' ) ) {
						wp_send_json_error( array( 'mess' => __( 'Nonce is invalid', 'wp-org-plugin-stats' ) ) );
					}
					...
					wp_send_json_success(
						array(
							'mess' => __( 'Activate success', 'wp-org-plugin-stats' ),
						)
					);
				}
			} catch ( \Exception $ex ) {
				wp_send_json_error(
					array(
						'mess' => __( 'Error exception.', 'wp-org-plugin-stats' ),
						array(
							'error' => $ex,
						),
					)
				...
				wp_send_json_error(
					array(
						'mess' => __( 'Error.', 'wp-org-plugin-stats' ),
						array(
							'error' => $ex,
						),
					)
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltwporgst_recommended_upgrade_plugin() {
			try {
				require_once ABSPATH . 'wp-admin/includes/plugin-install.php';
					...
					if ( 'install' === $type ) {
						...
						if ( $plugin_data && ! is_wp_error( $plugin_data ) ) {
							...
							if ( is_wp_error( $active_plugin ) ) {
								...
								wp_send_json_success(
									array(
										'mess' => __( 'Install success', 'wp-org-plugin-stats' ),
									)
								);
							}
						} else {
						...
						if ( is_wp_error( $result ) ) {
							...
							wp_send_json_success(
								array(
									'mess'   => __( 'Update success', 'wp-org-plugin-stats' ),
									'active' => $is_active,
								)
							);
						}
