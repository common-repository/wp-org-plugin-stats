...
use JLTWPORGST\Inc\Classes\Notifications\Base\Date;
...
class Pro_Upgrade {
	...
	public function __construct() {
		$this->slug = Helper::jltwporgst_slug_cleanup();
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jltwporgst_dashboard_widget',                          // Widget slug.
			esc_html__( 'wp-org-plugin-stats News & Updates', 'wp-org-plugin-stats' ), // Title.
		...
		$example_widget_backup = array( 'jltwporgst_dashboard_widget' => $default_dashboard['jltwporgst_dashboard_widget'] );
		unset( $default_dashboard['jltwporgst_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="jltwporgst-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="jltwporgst-dashboard-promo" style="--jltwporgst-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="wp-org-plugin-stats Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="jltwporgst-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'wp-org-plugin-stats' );
		...
		<div class="jltwporgst-dashboard_footer">
			<ul>
				<li class="jltwporgst-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'wp-org-plugin-stats' ); ?>
                        </span>
		...
		<style>
			...
			.jltwporgst-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.jltwporgst-rss-widget .jltwporgst-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.jltwporgst-rss-widget .jltwporgst-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.jltwporgst-rss-widget .jltwporgst-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.jltwporgst-rss-widget .jltwporgst-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.jltwporgst-rss-widget .jltwporgst-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jltwporgst_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jltwporgst_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jltwporgst_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTWPORGST_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jltwporgst_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jltwporgst_sheet_promo_data', $remote_data );
			update_option( 'jltwporgst_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jltwporgst_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jltwporgst_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="jltwporgst-popup jltwporgst-upgrade-popup" id="jltwporgst-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="jltwporgst-popup-overlay"></div>

			<div class="jltwporgst-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltwporgst-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">

				<!-- close  -->
				<div class="jltwporgst-popup-modal-close popup-dismiss">×</div>

				<!-- content section  -->
				<div class="jltwporgst-popup-modal-footer">

					<!-- countdown  -->
					<div class="jltwporgst-popup-countdown" style="display: none;">
						<span class="jltwporgst-popup-countdown-text">Deal Ends In</span>
						<div class="jltwporgst-popup-countdown-time">
							<div>
					...
					<!-- button  -->
					<a class="jltwporgst-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#jltwporgst-popup'),
				plugin_data = <?php echo json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".jltwporgst-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".jltwporgst-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".jltwporgst-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".jltwporgst-popup-countdown").hide(0);
				}
