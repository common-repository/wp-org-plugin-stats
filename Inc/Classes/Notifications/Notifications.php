...
class Notifications {
	...
	public function __construct() {
		...
		add_action( 'jltwporgst_display_notice', array( $this, 'display_notice' ), 10, 2 );
		add_action( 'jltwporgst_display_popup', array( $this, 'display_popup' ), 10, 2 );

		add_action( 'wp_ajax_jltwporgst_notification_action', array( $this, 'notification_action' ) );
	}
	...
	public function notification_action() {
		check_ajax_referer( 'jltwporgst_notification_nonce' );
	...
	public function setup_notifications_by_type( $type ) {
		...
		// Block if necessary .
		$notification_last_fired = get_option( "jltwporgst_{$type}_last_interact" );
		...
		do_action( "jltwporgst_display_{$type}", $notification, $trigger_time );
	}
	...
	public function display_popup( $popup, $trigger_time ) {
		...
		<div class="jltwporgst-popup" id="jltwporgst-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1">

			<div class="jltwporgst-popup-overlay"></div>

			<div class="jltwporgst-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltwporgst-popup-color: <?php echo esc_attr( $popup->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="jltwporgst-popup-modal-footer">
					...
					<div class="jltwporgst-popup-countdown" style="display: none;">
						<span class="jltwporgst-popup-countdown-text"><?php echo esc_html__( 'Deal Ends In', 'wp-org-plugin-stats' ); ?></span>
						<div class="jltwporgst-popup-countdown-time">
							<div>
								<span data-counter="days">00</span>
								<span><?php echo esc_html__( 'Days', 'wp-org-plugin-stats' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="hours">00</span>
								<span><?php echo esc_html__( 'Hours', 'wp-org-plugin-stats' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="minutes">00</span>
								<span><?php echo esc_html__( 'Minutes', 'wp-org-plugin-stats' ); ?></span>
							</div>
							<span>:</span>
							<div>
								<span data-counter="seconds">00</span>
								<span><?php echo esc_html__( 'Seconds', 'wp-org-plugin-stats' ); ?></span>
							</div>
						</div>
					</div>

					<!-- button  -->
					<a class="jltwporgst-popup-button" target="_blank" href="<?php echo esc_url( $popup->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $popup->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			function jltwporgst_popup_action(evt, $this, $action_type) {

				evt.preventDefault();

				$this.closest('.jltwporgst-popup').fadeOut(200);
			...
			// Notice Dismiss
			jQuery('body').on('click', '.jltwporgst-popup .popup-dismiss', function(evt) {
				jltwporgst_popup_action(evt, jQuery(this), 'dismiss');
			});

			// Notice Disable
			jQuery('body').on('click', '.jltwporgst-popup .popup-disable', function(evt) {
				jltwporgst_popup_action(evt, jQuery(this), 'disable');
			});
		</script>
